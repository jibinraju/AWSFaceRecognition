...
private extension CameraViewController {
    ...
    @IBAction func captureImage(_ sender: UIButton) {
        ...
        cameraController.captureImage {(image, error) in
            ...
            if self.isEnrollment == true {
                self.awsService.sendData(image: image, completion: { (response, error) in
                    if error != nil {
                        debugPrint(error.debugDescription as Any)
                    } else {
                        for faceRecord in (response?.faceRecords)! {
                            print("Face added: Faceid is \(String(describing: faceRecord.face?.faceId))")
                        }
                    }
                })
            } else {
                self.awsService.findData(image: image, completion: { (response, error) in
                    if error != nil {
                        debugPrint(error.debugDescription as Any)
                    } else {
